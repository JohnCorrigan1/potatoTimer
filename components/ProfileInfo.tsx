...
const ProfileInfo: React.FC<{ name: string, lifeTimeHours: number, workTime: number, breakTime: number }> = (props) => {
...
<h1 className="text-2xl font-semibold mr-auto ml-auto">Display name</h1>
            ...
            <div>
            <h1 className="text-2xl font-semibold mr-auto ml-auto">Work time (min)</h1>
<div className="flex justify-between border-2 border-base-100 p-3 rounded-lg items-center">
                {isEdditingWork && (
                    <>
                        <input onChange={handleWorkTime} value={workTime} className="input input-bordered w-1/2" type="text" placeholder={props.name} />


                        <button className="btn btn-secondary text-lg font-semibold w-1/4" onClick={handleWorkSave}>Save</button>
                    </>)}
                {!isEdditingWork && (
                    <>

                        <h1 className="text-3xl font-bold">{workTime}</h1>
                        <button className="btn btn-secondary text-lg font-semibold w-1/4" onClick={() => setIsEdditingWork(true)}>Edit</button>
                    </>
                )}
            ...
            <div>
            <h1 className="text-2xl font-semibold mr-auto ml-auto">Break time (min)</h1>
<div className="flex justify-between border-2 border-base-100 p-3 rounded-lg items-center">
                {isEdditingBreak && (
                    <>
                        <input onChange={handleBreakTime} value={breakTime} className="input input-bordered w-1/2" type="text" placeholder={props.name} />

                        <button className="btn btn-secondary text-lg font-semibold w-1/4" onClick={handleBreakSave}>Save</button>
                    </>)}
                {!isEdditingBreak && (
                    <>

                        <h1 className="text-3xl font-bold">{breakTime}</h1>
                        <button className="btn btn-secondary text-lg font-semibold w-1/4" onClick={() => setIsEdditingBreak(true)}>Edit</button>
                    </>
                )}
