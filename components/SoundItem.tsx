...
const SoundItem: React.FC<{
  setCurrentSound: Dispatch<SetStateAction<string>>;
  ...
  return (
    <div className="flex flex-col w-full items-center justify-center gap-5 p-5 border h-56">
      <h1 className="text-primary text-xl font-bold">{props.name}</h1>
      <button onClick={handleSound} className="btn btn-primary w-1/2">
        Try Me
      </button>
      <button onClick={handleSelect} className="btn btn-seconday w-1/2">
        Select
      </button>
